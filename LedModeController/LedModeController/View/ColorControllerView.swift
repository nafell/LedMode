...
//    private var tanoshii: ColorModel = ColorModel(id: "PaleYellow", label: "楽しい", red: 250, green: 244, blue: 144)
    private var tanoshii: ColorModel = ColorModel(id: "PaleYellow", label: "楽しい", red: 0, green: 255, blue: 31)
...
//    private var kanashii: ColorModel = ColorModel(id: "DarkBlue", label: "悲しい", red: 86, green: 60, blue: 251)
    private var kanashii: ColorModel = ColorModel(id: "DarkBlue", label: "悲しい", red: 0, green: 100, blue: 255)
    ...
    private var largeButtonColorArray: ColorPair
    private var colorArrays: [ColorPair]
    
    init(viewModel: AppViewModel) {
        self.viewModel = viewModel
        largeButtonColorArray = ColorPair(tanoshii, kanashii)
        colorArrays = [ColorPair(ureshii, hazukashii), ColorPair(anshin, ikari), ColorPair(off, on)] // ColorPair(suki, sabishii), 
    }
    ...
    var body: some View {
        VStack{
            ...
            // 大きいボタン
            HStack{
                ColorButton(color:largeButtonColorArray.colorLeft, action:viewModel.sendColor, isSelected: viewModel.selectedColor.id == largeButtonColorArray.colorLeft.id, isLarge: true)
                ColorButton(color:largeButtonColorArray.colorRight, action:viewModel.sendColor, isSelected: viewModel.selectedColor.id == largeButtonColorArray.colorRight.id, isLarge: true)
            }
            // 通常ボタン
            ForEach(self.colorArrays) { colorArray in
                HStack{
                    ColorButton(color:colorArray.colorLeft, action:viewModel.sendColor, isSelected: viewModel.selectedColor.id == colorArray.colorLeft.id)
                    ColorButton(color:colorArray.colorRight, action:viewModel.sendColor, isSelected: viewModel.selectedColor.id == colorArray.colorRight.id)
                }
            }
...
#Preview {
    ColorControllerView(viewModel: AppViewModel(bleService: BleService()))
}
